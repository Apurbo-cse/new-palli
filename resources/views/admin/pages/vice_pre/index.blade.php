            ...
            <div class="panel panel-primary">
                <div class="panel-heading text-end">
                    <h3 class="panel-title">
                        <button class="btn btn-primary  mx-5"><a href="{{route('admin.vice-president.index')}}" class="text-white">All</a></button>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                        ...
                        @foreach($v_p_members as $vice_pre)
                            <tr>
                                <td>{{$vice_pre->id}}</td>
                                <td>{{$vice_pre->name}}</td>
                                <td>{{$vice_pre->designation}}</td>
                                <td>{{$vice_pre->job}}</td>
                                <td>{{$vice_pre->job_location}}</td>
                                <td class="text-center"><img src="{{ asset($vice_pre->image) }}"  width="20%" alt=""></td>
                                <td>
                                    @if ($vice_pre->status == 0)
                                ...
                                <td class="d-flex">
                                    <a class="btn btn-info d-inline-block" href="{{ route('admin.vice-president.edit',$vice_pre->id) }}"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="d-inline-block pull-right" method="post" action="{{ route('admin.vice-president.destroy',$vice_pre->id) }}">
                                        @csrf
                                        @method('delete')
