    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-header-title">
                <h4 class="pull-left page-title">developer</h4>
                <ol class="breadcrumb pull-right">
                    <li><a href="">Dashboard</a></li>
                    <li><a href="{{route('admin.developer.create')}}">developer Create</a></li>
            ...
            <div class="panel panel-primary">
                <div class="panel-heading text-end">
                    <h3 class="panel-title">
                        <button class="btn btn-primary  mx-5"><a href="{{route('admin.developer.index')}}" class="text-white">All</a></button>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                        ...
                        @foreach($developers as $developer)
                            <tr>
                                ...
                                <td>{{$developer->job}}</td>
                                <td>{{$developer->job_location}}</td>

                                <td>{{$developer->phone}}</td>
                                <td>{{$developer->email}}</td>
                                <td>{{$developer->facebook}}</td>
                                <td>{{$developer->linkedin}}</td>
                                <td class="text-center"><img src="{{ asset($developer->image) }}"  width="20%" alt=""></td>
                                <td>
                                ...
                                <td class="d-flex">
                                    <a class="btn btn-info d-inline-block" href="{{ route('admin.developer.edit',$developer->id) }}"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    <form class="d-inline-block pull-right" method="post" action="{{ route('admin.developer.destroy',$developer->id) }}">
                                        @csrf
                                        @method('delete')
